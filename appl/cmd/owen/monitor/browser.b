...
{
	b : Browse;
	b.rootpane = ".fbrowse" + string browseid++;
	b.top = top;
	b.tkchan = tkchanname;
	...
	b.released = 1;
	makepane0(top, b.rootpane);

	makepane1(top, b.rootpane);
	tkcmd(top, "bind "+b.rootpane+".lmov <Button-1> {send "+b.tkchan+" movdiv %X}");

	tkcmd(top, "label "+b.rootpane+".l -text { }  -anchor w -width 0" +
		" -font /fonts/charon/plain.normal.font");
	tkcmd(top, b.rootpane+".l configure -height "+tkcmd(top, b.rootpane+".l cget -height"));
	tkcmd(top, "grid "+b.rootpane+".l -row 0 -column 0 -sticky ew -pady 2 -columnspan 4");
	rb := ref b;
...
{
	scrval := tkcmd(b.top, b.rootpane+".sy1 get");
	p := isat(scrval, " ");
	p1 := b.pane1;
	...
	tkcmd(b.top, b.rootpane+".c1 yview moveto "+scrval[:p]+"; update");
}
...
Browse.gotoselectfile(b: self ref Browse, file: File): string
{
	...
	if (!File.eq(file, dir)) {
		slaves := tkcmd(b.top, "grid slaves "+b.rootpane+".fl2");
...
pane1see(b: ref Browse)
{
	...
	if (f == "")
		return;
	x1 := int tkcmd(b.top, f+" cget -actx") - int tkcmd(b.top, b.rootpane+".fl2 cget -actx");
	y1 := int tkcmd(b.top, f+" cget -acty") - int tkcmd(b.top, b.rootpane+".fl2 cget -acty");
	x2 := x1 + int tkcmd(b.top, f+" cget -actwidth");
	y2 := y1 + int tkcmd(b.top, f+" cget -actheight");
	tkcmd(b.top, sys->sprint("%s.c2 see %d %d %d %d", b.rootpane,x1,y1,x2,y2));
}
...
Browse.opendir(b: self ref Browse, file: File, tkpath: string, action: int): int
{
	...
	if ((action == OPEN || action == TOGGLE) && curr == "+") {
		...
		seeframe(b.top, tkpath, b.rootpane);
		b.addopened(file, 1);
		setbrowsescrollr(b);
		return 1;
	}
...
Browse.changeview(b: self ref Browse, nopanes: int)
{
	if (b.nopanes == nopanes)
		return;
	if (nopanes == 1) {
		b.pane0width = tkcmd(b.top, b.rootpane+".c1 cget -actwidth") + " " +
						tkcmd(b.top, b.rootpane+".c2 cget -actwidth");
		tkcmd(b.top, "grid forget "+b.rootpane+".sx2 "+b.rootpane+".c2 "+b.rootpane+".lmov");
		tkcmd(b.top, "grid columnconfigure "+b.rootpane+" 3 -weight 0");
	}
	else {
		(nil, wlist) := sys->tokenize(b.pane0width, " ");
		tkcmd(b.top, "grid columnconfigure "+b.rootpane+" 1 -weight "+hd wlist);
		tkcmd(b.top, "grid columnconfigure "+b.rootpane+" 3 -weight "+hd tl wlist);

		tkcmd(b.top, "grid "+b.rootpane+".sx2 -row 3 -column 3 -sticky ew");
		tkcmd(b.top, "grid "+b.rootpane+".c2 -row 2 -column 3 -sticky nsew");
		tkcmd(b.top, "grid "+b.rootpane+".lmov -row 2 -column 2 -rowspan 2 -sticky ns");
	}
	b.nopanes = nopanes;
}
...
{
	h := tkcmd(b.top, b.rootpane+".fl cget -height");
	w := tkcmd(b.top, b.rootpane+".fl cget -width");
	tkcmd(b.top, b.rootpane+".c1 configure -scrollregion {0 0 "+w+" "+h+"}");
	if (b.nopanes == 2) {
		h = tkcmd(b.top, b.rootpane+".fl2 cget -height");
		w = tkcmd(b.top, b.rootpane+".fl2 cget -width");
		tkcmd(b.top, b.rootpane+".c2 configure -scrollregion {0 0 "+w+" "+h+"}");
	}
}

seeframe(top: ref Tk->Toplevel, frame: string, rootpane: string)
{
	x := int tkcmd(top, frame+" cget -actx") - int tkcmd(top, rootpane+".fl cget -actx");
	y := int tkcmd(top, frame+" cget -acty")  - int tkcmd(top, rootpane+".fl cget -acty");
	w := int tkcmd(top, frame+" cget -width");
	h := int tkcmd(top, frame+" cget -height");
	wc := int tkcmd(top, rootpane+".c1 cget -width");
	hc := int tkcmd(top, rootpane+".c1 cget -height");
	if (w > wc)
		w = wc;
	if (h > hc)
		h = hc;
	tkcmd(top, sys->sprint("%s.c1 see %d %d %d %d",rootpane,x,y,x+w,y+h));
}
...
{
	testl := 3 + len b.rootpane;
	if (len f < testl || f[:testl] != b.rootpane+".fl")
		return nil;
...
setroot(b: ref Browse, rlabel, root: string)
{
	b.root = root;
	b.rlabel = rlabel;
	makedir(b, File (root, nil), b.rootpane+".fl.f0", rlabel, "0");
	tkcmd(b.top, "grid forget "+b.rootpane+".fl.f0.lp");
}
...
Browse.defaultaction(b: self ref Browse, lst: list of string, rfile: ref File)
{
	...
	if (len lst > 1) {
		tkpath = hd tl lst;
		testl := 3 + len b.rootpane;
		if (len tkpath > testl && tkpath[:testl] == b.rootpane+".fl") {
			if (rfile == nil)
...
#		return;
	b.pane1 = dir;
	labelset(b, b.rootpane+".l", prevpath(dir.path+"/"));
	if (b.nopanes == 1)
		return;
	tkcmd(b.top, "destroy "+b.rootpane+".fl2; frame "+b.rootpane+".fl2 -bg white");
	tkcmd(b.top, b.rootpane+".c2 create window 0 0 -window "+b.rootpane+".fl2 -anchor nw");
	if (dir.path == nil) {
		setbrowsescrollr(b);
		return;
	}
...
#	tkcmd(b.top, "label "+b.rootpane+".fl2.l -text {"+path+"}");
	row := 0;
	col := 0;
	tkcmd(b.top, b.rootpane+".c2 see 0 0");
	ni := 0;
	n := (int tkcmd(b.top, b.rootpane+".c2 cget -actheight")) / 21;
	for (i := 0; i < len dirs; i++) {

		f := b.rootpane+".fl2.l"+string ni;
		if (hasdups)
...
makepane0(top: ref Tk->Toplevel, rootpane: string)
{
	pane0scr := array[] of {
		"frame "+rootpane,

		"scrollbar "+rootpane+".sy1 -command {"+rootpane+".c1 yview}",
		"scrollbar "+rootpane+".sx1 -command {"+rootpane+".c1 xview} -orient horizontal",
		"canvas "+rootpane+".c1 -yscrollcommand {"+rootpane+".sy1 set} -xscrollcommand {"+rootpane+".sx1 set} -bg white -width 50 -height 20 -borderwidth 2 -relief sunken -xscrollincrement 10 -yscrollincrement 21",
		"grid "+rootpane+".sy1 -row 2 -column 0 -sticky ns -rowspan 2",
		"grid "+rootpane+".sx1 -row 3 -column 1 -sticky ew",
		"grid "+rootpane+".c1 -row 2 -column 1 -sticky nsew",
		"grid rowconfigure "+rootpane+" 2 -weight 1",
		"grid columnconfigure "+rootpane+" 1 -weight 2",
	};
	tkcmds(top, pane0scr);
}

makepane1(top: ref Tk->Toplevel, rootpane: string)
{
	pane1scr := array[] of {
	#	rootpane+".c1 configure -width 146",
		"frame "+rootpane+".fl2 -bg white",
		"label "+rootpane+".fl2.l -text {}",
		"scrollbar "+rootpane+".sx2 -command {"+rootpane+".c2 xview} -orient horizontal",
		"label "+rootpane+".lmov -text { } -relief sunken -borderwidth 2 -width 5",
		
		"canvas "+rootpane+".c2 -xscrollcommand {"+rootpane+".sx2 set} -bg white -width 50 -height 20 -borderwidth 2 -relief sunken -xscrollincrement 10 -yscrollincrement 21",
		rootpane+".c2 create window 0 0 -window "+rootpane+".fl2 -anchor nw",
		"grid "+rootpane+".sx2 -row 3 -column 3 -sticky ew",
		"grid "+rootpane+".c2 -row 2 -column 3 -sticky nsew",
		"grid "+rootpane+".lmov -row 2 -column 2 -rowspan 2 -sticky ns",
		"grid columnconfigure "+rootpane+" 3 -weight 3",
	};
	tkcmds(top, pane1scr);
}
...
{
	tk->cmd(b.top, "destroy "+b.rootpane+".fl");
	tkcmd(b.top, "frame "+b.rootpane+".fl -bg white");
	tkcmd(b.top, b.rootpane+".c1 create window 0 0 -window "+b.rootpane+".fl -anchor nw");
...
Browse.showpath(b: self ref Browse, on: int)
{
	...
	if (on) {
		b.showpathlabel = 1;
		if (b.pane1.path != nil)
			labelset(b, b.rootpane+".l", prevpath(b.pane1.path+"/"));
	}
	else {
		b.showpathlabel = 0;
		tkcmd(b.top, b.rootpane+".l configure -text {}");
	}	
}
...
{
	x1 := int tkcmd(b.top, b.rootpane+".lmov cget -actx");
	x2 := x1 + int tkcmd(b.top, b.rootpane+".lmov cget -width");
	diff := 0;
	if (x < x1)
		diff = x - x1;
	if (x > x2)
		diff = x - x2;
	if (abs(diff) > 5) {
		w1 := int tkcmd(b.top, b.rootpane+".c1 cget -actwidth");
		w2 := int tkcmd(b.top, b.rootpane+".c2 cget -actwidth");
		if (w1 + diff < 36)
			diff = 36 - w1;
		if (w2 - diff < 36)
			diff = w2 - 36;
		...
		w2 -= diff;
		# sys->print("w1: %d w2: %d\n",w1,w2);
		tkcmd(b.top, "grid columnconfigure "+b.rootpane+" 1 -weight "+string w1);
		tkcmd(b.top, "grid columnconfigure "+b.rootpane+" 3 -weight "+string w2);
	}
}
