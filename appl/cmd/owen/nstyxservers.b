...
Styxserver.new(fd: ref Sys->FD, t: ref Navigator, rootpath: big): (chan of ref Tmsg, ref Styxserver)
{
	...
	srv := ref Styxserver(fd, array[CHANHASHSIZE] of list of ref Fid, t, rootpath, 0, nil, chan[1] of int, nil);
...
Styxserver.attach(srv: self ref Styxserver, m: ref Tmsg.Attach): ref Fid
{
	...
	c.session.uname = m.uname;
	c.session.aname = m.aname;
	c.path = d.qid.path;
	c.qtype = d.qid.qtype;
...
walk1(srv: ref Styxserver, c: ref Fid, name: string): (ref Sys->Dir, string)
{
	n := srv.t;
	(d, err) := n.stat(c.path);
	if(d == nil)
	...
	if((d.qid.qtype & Sys->QTDIR) == 0)
		return (nil, Enotdir);
	if(!openok(srv, c, Styx->OEXEC, d.mode, d.uid, d.gid))
		return (nil, Eperm);
...
Styxserver.canopen(srv: self ref Styxserver, m: ref Tmsg.Open): (ref Fid, int, ref Sys->Dir, string)
{
	...
	if(!openok(srv, c, m.mode, f.mode, f.uid, f.gid))
		return (nil, 0, nil, Eperm);
	if(m.mode & Sys->ORCLOSE) {
		...
		if(dir == nil || dir.qid.path == f.qid.path && dir.qid.qtype == f.qid.qtype ||	# can't remove root directory
		   !openok(srv, c, Sys->OWRITE, dir.mode, dir.uid, dir.gid))
			return (nil, 0, nil, Eperm);
		mode |= Sys->ORCLOSE;
	}
	return (c, mode, f, err);
...
Styxserver.cancreate(srv: self ref Styxserver, m: ref Tmsg.Create): (ref Fid, int, ref Sys->Dir, string)
{
	...
	if(!openok(srv, c, Sys->OWRITE, d.mode, d.uid, d.gid))
		return (nil, 0, nil, Eperm);
	...
	f.name = m.name;
	f.uid = c.session.uname;
	f.muid = c.session.uname;
	f.gid = d.gid;
	f.dtype = d.dtype;
	f.dev = d.dev;
...
Styxserver.newfid(srv: self ref Styxserver, fid: int): ref Fid
{
	...
	c.fid = fid;
	c.doffset = (0, 0);
	c.session = ref Session(nil, nil, 1, -1);
...
openok(srv: ref Styxserver, fid: ref Fid, omode, perm: int, fuid, fgid: string): int
{
	t := access[omode & 3];
	if(omode & Sys->OTRUNC){
		if(perm & Sys->DMDIR)
	...
	uname := fid.session.uname;
	if(uname == fuid && (t&perm) == t)
		return 1;
	if((t&(perm<<3))==t){
		if(srv.ingroup != nil){
			if(srv.ingroup(srv, fid, fgid))
				return 1;
		}else if(uname == fgid)
			return 1;
	}
